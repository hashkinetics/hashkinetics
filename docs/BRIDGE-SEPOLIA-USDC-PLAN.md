...
> **Status 2026-09-14:** B1 is LIVE on the testnets since 2026-09-09 (both legs, docs/BRIDGE-GUIDE.md; 1-of-1 attestor, per-day cap, pause). B2 (attested mint in consensus) and B3 (light-client bridge) are not started.
...
**Build status (2026-09-09, in tree — receipts pending Yadu's runs, `ops/B1-RUNBOOK.md`):** B1.0 `bridge/contracts` (HKVault + HKWrapped over a shared `AttestorSet` + `DailyLimit`; 26 Foundry tests; `script/Deploy.s.sol`) · B1.2 the service as `hk-node attest-serve` (`chain/crates/hk-node/src/attest.rs` + `eth.rs`: the Ethereum client written by hand — keccak/ABI/RLP/EIP-1559/EIP-712 in ~500 readable lines, no alloy), with `attest-cosign` (a second attestor that re-checks the burn on its own node before signing), `attest-ledger` (list/resolve) and `attest-key-new`; the ledger is an append-only fsynced JSONL (write-ahead, not sqlite — same guarantee, greppable) · `chain/gate-b1.sh` (11 sections: lock→mint, shielded USDC.sep, burn→unlock, restart replay, kill -9 mid-flow held for review, paused/cap queues, the reverse leg both ways, 2-of-2 with a cosigner that refuses a wrong amount, an unbridgeable burn, reconciliation) · `ops/hk-attest.service` + `ops/attest.toml.example`. Reverse leg (§7): wraps the HK-issued asset `HKT` (bridge-issued, `mfps`) as `wHKT.sep` — the genesis test asset has no memo/issuer path.
...
- **Audit.** The vault and the service join the CertiK scope only if they are meant for mainnet value; the POC is explicitly out of the scope-freeze tag (not yet named; it follows B2 and C3.A) and stays a testnet feature until audited.
...
- Wrapping a mainnet asset on Ethereum is a policy decision for mainnet time; the machinery is built and rehearsed on testnet with an HK-issued asset (B1.4). **Decision 2026-09-09:** a transparent `Transfer` carries no memo, so the genesis test asset cannot name a Sepolia destination; the reverse leg therefore wraps `HKT` — an asset the bridge issuer registers (`mfps`) and mints as a float — via the X1 primitive that exists: burn-with-destination on HK → `HKWrapped.mint` on Sepolia; `HKWrapped.burn(hkAccount)` → issuer mint on HK. The same path serves any future issued asset (a stablecoin issued on HK, or an issued representation of HKN).
