...
> **Status 2026-09-14:** written against v0.15.0; the audited surface has since grown by V1 set changes, X1 assets, G1 bootstrap weights, R15 signing session, P6 pools per asset and the vendored gossipsub patch (v0.19.3); the B1 bridge (Sepolia contracts + the attest service, client-only, 1-of-1) sits outside the consensus scope until B2 puts attested minting in consensus. This document is refreshed before the scope-freeze tag is named (after B2 and C3.A). Nothing is audited yet; CertiK is engaged.
