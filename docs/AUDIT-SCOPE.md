...
> **Status 2026-09-23:** written against v0.15.0; the in-scope surface has since grown by V1 set changes, X1 assets, G1 bootstrap weights, R15 signing session, P6 pools per asset and the vendored gossipsub patch (v0.19.3) and R16 (v0.19.4: hashsig directory fsync + upward resume); the B1 bridge (Sepolia contracts + the attest service, client-only, 1-of-1) sits outside the consensus scope until B2 puts attested minting in consensus. This document is refreshed before the scope-freeze tag is named (after B2 and C3.A). Nothing is audited yet; CertiK is engaged.
...
4. **Leaf-index = nonce** — one-time account keys can't be reused (L-ratchet now); the
   consensus signer is guarded locally by reserve-then-sign (durable since v0.19.4) — no consensus-side reuse check yet.
...
(genesis-pinned vk hashes enforced on testnet-1; an unpinned genesis only warns — devnet posture) · one pool per asset since v0.19.0 (P6 — `docs/P6-MULTI-ASSET-POOL.md`; X1's per-asset `pool_eligible` decides which assets get a pool; before v0.19.0 the single pool pinned ONE asset) · issued-asset policy and issuer immutable after registration (no on-chain rotation of an issuer key yet — X2/K2) · no attested mint yet (every mint is issuer-signed; X2) · wallet state
...
Two firms (one circuit-specialist, one systems).
