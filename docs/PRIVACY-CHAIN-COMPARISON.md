...
| Throughput (as stated by us / them) | 274 tx/s sustained, 4-validator lab chain, storm harness (m; the public-testnet run is pending) | ~ | ~ | ~ | ~ | ~ | ~ | ~ thousands (Tendermint-class) | ~ | ~ | ~ |
...
| Independent audits | **none yet** — CertiK engaged; scope-freeze tag not yet named; mainnet gated behind the audits | many, over a decade | many (NCC, Least Authority, QED-it…) | several (Lelantus/Spark audited) | inherits Sapling audits | several | several | several | several | several | ~ |
| Governance today | founders decide alone under bootstrap governance (16 of 23 power) — said on the site; handover by dated milestone | community/PoW | ZF/ECC/Shielded Labs, dev fund | community + masternodes | community | masternode votes | on-chain Politeia | on-chain | on-chain | Horizen DAO | masternode votes |
...
5. **They have what we do not:** years of mainnet, audits, liquidity, wallets in the wild. HashKinetics is a testnet with 11 seats, a bridge with one attestor key, and an audit that has not happened. The site must say this in the same table, not in a footnote.
