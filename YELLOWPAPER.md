...
## 19 · Addenda — rules shipped after v1.0 (normative; v1.3 · 2026-09-14)
...
*Addenda v1.3 (2026-09-14). §19.2–19.3 stand as written: on testnet-1 the fee is bound in the genesis — `FEE_MICRO = 100`, `fee_from_height = 1`, burned on success — and has been charged on every envelope since block 1. §19.10 stands as written; §19.13 extends it to the pool.*

**19.12 Bootstrap weights and the handover (v0.18.1, G1; activation at height 110,000, committed 2026-09-06).** Constants: `G1_HEIGHT` (testnet-1: 110,000; hard-wired by chain id — a devnet reads `HK_G1_HEIGHT`, unset = no activation) and `G1_POWER` (testnet-1: 4). At height `G1_HEIGHT` every node re-weights each **genesis seat** — a root listed in `genesis.validators[]` that is still seated — to `power = G1_POWER` in place (operational key, epoch and address untouched), effective for the certificate of `G1_HEIGHT + 1` and recorded in the per-height set history of §19.6 exactly as a certificate would be; no proposal, no signatures — the rule is applied identically on a live commit, on replay from the store and on a sync from genesis (a snapshot taken after the height already carries the weights). A genesis seat removed before the height is not re-seated; a seat admitted by certificate is never touched. The `change` union of §19.9 gains `SetPower { root_pk, voting_power }` (sign-bytes variant tag `0x03`, distinguishing it from `Admit`/`Remove` over the same root; normative layout in `hk-consensus::setchange`): `voting_power ≥ 1` (use `Remove` to unseat), idempotent when the power already matches, the set's ordering by power then address shifting the proposer schedule at the height exactly as an admission does; the approval rule, chain-id binding and window of §19.9 are unchanged. This certificate is the handover: the founders' weight is lowered by it, one seat per change, each with a receipt. §19.9's sentence "Voting power is 1 per seat on testnet-1" is amended: from 110,001 the four genesis seats weigh 4 and every seat admitted by certificate weighs 1 — on 2026-09-14 eleven seats, Σ power 23, quorum `⌊2·23/3⌋ + 1 = 16`, so the founding seats decide alone and admitted seat #11 (2026-09-12, block 379,143) was the last admission possible on founding approvals only (`3·16 > 2·23` holds; `3·16 > 2·24` does not). Activation rule: every node ≥ v0.18.1 before 110,000 (a node still on ≤ v0.17, or on the withdrawn v0.18.0 which named 200,000, counts power 1 per seat, cannot make a quorum of the founding votes, and stops following). `hk_getValidators` reports `founding_power`, `external_power`, `quorum_power`, `max_absent_power`, `founders_alone_decide`, `bootstrap { height, founding_power, active }` and `genesis: bool` per seat. `Val` remains outside `C(Σ)`.

**19.13 One shielded pool per asset (v0.19.0, P6; activation at height 190,000, committed 2026-09-09).** `Σ` gains `Pools : Asset → PoolState` beside the legacy pool of §9 (`Pool`, unchanged), each with its own commitment tree, anchor window, nullifier set and conservation total; constant `P6_HEIGHT` (testnet-1: 190,000, hard-wired by chain id; devnets `HK_P6_HEIGHT`, unset = 0). **Routing.** A `MintToPool` of asset `a` goes to `Pool` when `a` is the legacy pool's pinned asset or the pool is unpinned (v1, byte for byte); otherwise, at `h ≥ P6_HEIGHT`, for `a ∈ dom(Reg)` with `policy.pool_eligible`, to `Pools[a]`, created on first use; before `P6_HEIGHT` a second asset is refused exactly as v1 (`pool asset mismatch`). A `ShieldedSpend` goes to the pool whose recent anchors contain its `anchor` (non-empty roots are unique across pools); its unshield credit is in that pool's asset. Every pool seals its anchor at block end. **Statements.** The note commitment, the spend statement (§9, circuit v3) and the aggregation coverage of §10 are unchanged and asset-agnostic — a note is value units of the pool it lives in, membership is proven against one tree, so a note can never cross pools; no new vk pins (§15). **Commitment.** `enc(Σ)` appends a `0xB6` section only once a second pool exists, so pre-P6 and P6 nodes agree until the first second-asset shield, which only a P6 node accepts — hence the activation rule: every node ≥ v0.19.0 before 190,000 (an older node islands at that shield, not at the height). **Invariant I5′ (§19.10) counts every pool:** the term `[Pool.asset = a]·total_shielded` becomes the sum of `total_shielded` over every pool holding `a`. Snapshot image v4 (a v3 image restores with empty pools and is refused on `app_hash` if the chain had any — never silent). Read-back: `asset?` on the pool methods (absent = legacy, so pre-P6 wallets are unchanged), `hk_getPools`, `hk_getAsset.shielded`. On testnet-1 the second pool opened at height 214,097 with `USDC.sep`.

**19.14 Exhibit — the B1 bridge (v0.19.1, 2026-09-09; client-only, not a consensus rule).** `USDC.sep` (registered at 170,698; 6 decimals; policy `mfps`) is an asset of §19.10 whose issuer mints `AssetMint` against Sepolia USDC locked in `HKVault` (`0x989Cb35485d16c7b19Dff890b617984EeD9970aF`, Sepolia 11155111) after Ethereum finality, and whose `AssetBurn` with a 20-byte destination is released by `unlock` under t-of-n EIP-712 attestor signatures (t = n = 1 today); the reverse leg wraps the issued `HKT` as `wHKT.sep` (`HKWrapped`, `0x24159bE1577f3016AD64beA7D1430e5F2313DB26`). Reconciliation, checked off-chain and on the bridge page: `reserves = supply − burned (+ pending)`; shielding moves nothing in `supply − burned`. **Trust:** a 1-of-1 founder-run attestor, ECDSA on the Ethereum side — labelled as such on every surface; the chain verifies nothing about Sepolia, and `USDC.sep` is never called "USDC". The consensus-verified attested mint (`AssetMintAttested`; B2) is designed, not built.

**19.15 Implementation note — gossipsub subscriptions on reconnect (v0.19.3, N2; non-consensus).** libp2p-gossipsub 0.49.5 announces a node's topic subscriptions only on the first connection to a peer id; a validator that restarted and re-dialed a peer before its previous connection had closed held that peer with an empty topic set and could publish nothing (`NoPeersSubscribedToTopic`) while receiving everything — every proposer slot committed in round 1. The vendored crate re-announces on every additional connection to a known peer and whenever a connection closes while another remains; the receiver keeps a set, so duplicates are harmless. Validity, `C(Σ)` and the wire of §13 are untouched; `hk_getPeers` gains `reconnects` and `last_connection_secs`. Operators restart with a gap (stop, 45 s, start; never two processes on one consensus key).

---

*HashKinetics yellowpaper v1.0 (2026-08-18) + §19 addenda v1.2 (2026-09-04: 19.9 validator-set changes, 19.10 issued assets, 19.11 receipts) + v1.3 (2026-09-14: 19.12 bootstrap weights and `SetPower`, 19.13 one pool per asset, 19.14 the B1 exhibit, 19.15 gossipsub note). Normative companion: the reference implementation and its test suite (hk-state 21 incl. the I5′ conservation tests, hk-node 19, hk-consensus 3 set-change tests, + 17 circuit at v0.15.0); `docs/SHIELDED-POOL-SPEC.md` for narrative protocol detail; the whitepaper for motivation, economics, and history.*
