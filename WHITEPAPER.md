...
- **Measured** numbers (proof latencies, block cadence, aggregate sizes, test counts, demo transcripts) originate from the 2026 development network on documented hardware (AMD Threadripper PRO 7995WX, NVIDIA RTX 5090, WSL2), recorded in the repository's engineering logs (`CHANGELOG.md`, `docs/CAPACITY-SHEET.md`, `zkvm-bakeoff/RESULTS.md`). They are facts about that environment.
...
The same certificate discipline admits and removes *seats* (v0.14.0): a set-change certificate approved by SLH-DSA root signatures from strictly more than two thirds of the current seats' voting power, bound to the chain id and to a commit-height window, rides an ordinary block and takes effect one height after it commits — so the validator set of a running chain changes without a new genesis, and a node syncing from genesis re-derives every set from the certificates alone. Mainnet replaces the approval rule with bonded self-admission and governance; the certificate shape stays.
...
A transparent account is an identifier, a balance map, a nonce, and a hash commitment to its current authentication key. Transactions open the commitment, sign the payload together with the commitment to the *next* key, and advance the nonce — the leaf-index-equals-nonce rule making one-time-ness a ledger property. Account creation (`Tx::AccountCreate`, v0.11.0), the flat protocol fee (v0.12.2; bound into the genesis since v0.13.0) and **issued assets** (v0.15.0: an asset registry whose entries are `H(issuer ‖ symbol)` — squat-proof like account ids — with an issuer-fixed policy of mint, freeze, pause and shield-eligibility, per-asset supply and burn counters inside the state commitment, and a single gate on every movement of a registered asset; the floor a stablecoin issuer needs before it will deploy, and the base of the attested-mint path) have shipped; the merkleized global commitment follows the hardening track (§16).
...
- **P3 — hardening to mainnet (in progress; testnet-1 live since 2026-09-02).** Public testnet with external operators (validator-set changes on the running chain shipped in v0.14.0, the first external seat admitted through them), the 30-day incident-free soak, load benches producing the capacity sheet of §13; the three audits (CertiK engaged); CASP envelope registry and IVMS-101 binding; the stablecoin issuance path (issued assets with issuer controls shipped in v0.15.0; attested mint next); TGE under the raise plan; **mainnet launch, shielded by default, into the AMLR Article 79 window** — with the compliance architecture as the headline.
...
13. HashKinetics repository: `HASHKINETICS-IMPLEMENTATION-PLAN.md` · `CHANGELOG.md` (every release with its gate and roll receipts) · `docs/CAPACITY-SHEET.md` · `docs/SHIELDED-POOL-SPEC.md` · `docs/TECHNOTE-CONSENSUS-GUARDED-STATEFUL-HASH-SIGNATURES.md` · `docs/AUDIT-SCOPE.md` · `zkvm-bakeoff/RESULTS.md` · market/design research 01–03 (2026-08-15, sourced and dated).

---

## 19 · Addendum — the protocol as shipped after v1.0 (2026-09-14)

The body above is v1.0 (2026-08-18) with the v0.15.0 re-stamp; the rules and exhibits below have shipped on testnet-1 since and are stated here rather than rewritten into the body. Each was gated on a devnet, released with a sha256 row, and rolled one voter at a time without a pause; the yellowpaper §19 carries the normative form.

- **Bootstrap governance (G1, v0.18.1, active at height 110,000 on 2026-09-06).** With external seats at power 1, a young set of six left the founders below two thirds — a set change needed an external co-signature from a seat days old, and two external machines down meant a halt. From the activation height every node re-weights the *genesis* seats to power 4 **by rule** — a published constant in the binary, applied identically on live commit, on replay and on a sync from genesis; not a certificate. The weight comes back to external seats by the **`SetPower` certificate** (the third set-change kind beside `Admit` / `Remove`, under the same >⅔ approval rule of §5.3): that certificate *is* the handover, and the day it commits starts the 30-day soak clock of §16. On 2026-09-14 the set is eleven seats — four founding at 4, seven external at 1 (Σ 23, quorum 16; the founders decide alone) — and seat #11 (2026-09-12) was the last the founders can admit without an external approval.
- **Issued assets (X1, v0.15.0) and one shielded pool per asset (P6, v0.19.0, active at height 190,000 on 2026-09-09).** §6.1's registry stands as written — an id only its issuer can claim, `H(issuer ‖ symbol)`; mint · burn · freeze · pause under a policy fixed at registration; per-asset supply in the state commitment. P6 gives every pool-eligible asset its own shielded pool beside the legacy pool of §7: a **shield routes by asset** (to the legacy pool for its pinned asset, otherwise to the asset's pool, created on first use), a **spend routes by anchor** (non-empty roots are unique across pools), and the note commitment and spend statement of §7.5 are **unchanged** — a note is value units of the pool it lives in and membership is proven against one tree, so a note cannot cross pools; no circuit change, no new vk pins. The anonymity set is per asset, as an issuer expects; the shared-anonymity-set design stays a later product decision. testnet-1's second pool opened at height 214,097 with bridged `USDC.sep`.
- **Exhibit — the B1 bridge (live on the testnets since 2026-09-09; §14.1's bridged-issuance-first path).** Sepolia USDC locked in `HKVault` (`0x989Cb354…9970aF`) is minted on testnet-1 as the issued asset `USDC.sep` by its issuer account after Ethereum finality; a burn naming a Sepolia address is released by `unlock`; the reverse leg wraps the issued `HKT` as `wHKT.sep` (`HKWrapped`, `0x24159bE1…13DB26`). The invariant `vault reserves = supply − burned` is checked live on the bridge page. **Trust label, said everywhere:** a **1-of-1 founder-run attestor** (ECDSA, because Ethereum verifies nothing else), and `USDC.sep` is never called "USDC". The consensus-verified attested mint (the path §6.1 calls the base of; B2) is designed, not built — the bridge is an exhibit of the asset rails, not the mainnet bridge.
- **The fee rule as a genesis fact (U4.b, v0.13.0).** testnet-1's genesis binds the protocol fee — **100 micro per envelope, burned, from height 1** — so a node's local configuration cannot differ from the network's and no activation height had to be coordinated; `fees_burned` is a component of the state commitment. Every transaction on testnet-1 has paid it since block 1.
- **Implementation note — gossipsub subscriptions on reconnect (N2, v0.19.3, 2026-09-13; not a rule).** libp2p-gossipsub 0.49.5 announces a node's topic subscriptions only on the first connection to a peer id; a validator that restarted and re-dialed before its previous socket died held its peer with an empty topic set and proposed in round 1 for eight hours while looking in sync. The vendored crate now re-announces on every additional connection and whenever a connection closes while another remains; validity and `C(Σ)` are untouched. Operators restart with a gap (stop, 45 s, start).
- **Where the network stands (2026-09-23).** Node v0.19.4 (consensus rules v0.19.0); ≈ 1,104,500 blocks at ~1.3 s per block; a consensus signature costs 179 µs (v0.18.2); every node verifies STARKs with a verify-only client (v0.17.0: 54–59 MiB resident); wallets on Windows (v0.15.0) and Android (v0.3.0) carry every registered asset. Nothing is audited yet; the audit campaign precedes any mainnet.

---

*HashKinetics · $HKN · whitepaper v1.0 (2026-08-18), status re-stamped v0.15.0 (2026-09-04: validator-set changes §5.3, issued assets §6.1, P3 progress §16); §19 addendum 2026-09-14 (G1 bootstrap weights + `SetPower`, P6 pools per asset, the B1 exhibit, the genesis-bound fee, N2). Confidential where so marked; not an offer.*
