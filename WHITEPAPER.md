...
- **Where the network stands (2026-09-14).** Node v0.19.3 (consensus rules v0.19.0); ≈ 462,000 blocks at ~1.3 s per block; a consensus signature costs 179 µs (v0.18.2); every node verifies STARKs with a verify-only client (v0.17.0: 54–59 MiB resident); wallets on Windows (v0.15.0) and Android (v0.3.0) carry every registered asset. Nothing is audited yet; the audit campaign precedes any mainnet.
