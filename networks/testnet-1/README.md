...
- Observers hold a validator keypair (from `keygen`) but are not in the validator
  set: you sync and verify, you don't vote. No stake, no cost, no GPU (testnet-1). At mainnet a node needs a Node Key — a software license, not a token; Node Keys are available at https://nodes.hashkinetics.org. Keep that
