...
- **Rotation hardening (R-series) is production-proven, not just built:** automatic threshold rotation has fired unattended across dozens of epochs; the peer-carried revival path has resurrected a fully-exhausted validator three times; certificates verify against the set as of their height, so syncing across rotation boundaries works (v0.10.7); and catch-up verification is parallel (v0.10.8 — a joining machine measured 2→71 blocks/min, converging on a chain that adds ~27). Still open and ledgered: engine resume at store tip (deep restarts rehydrate slowly and then trail the live chain — the interim cure is a snapshot-near-tip copy), a syncing non-validator still spends signing leaves it shouldn't (interim watchdog deployed), abstain-while-behind (a parked validator burns one-time leaves on futile rounds), and a tick-based rotation-threshold check.
