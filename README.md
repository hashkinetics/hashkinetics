...
`testnet-1: LIVE since 2026-09-02` · `node v0.18.0 · wallet v0.14.1` · `6 seats, 2 external` · `seats change on the running chain (v0.14.0)` · `bootstrap governance: genesis seats weigh 4 from height 200,000 (v0.18.0)` · `issued assets with issuer controls (v0.15.0)` · `spend proof: 1.24 s (GPU)` · `274 tx/s storm-measured` · `nothing is for sale`
