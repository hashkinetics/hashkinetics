...
- Explorer and wallet are public-testnet surfaces, not audited products: keys sit in plaintext JSON on your disk (passphrase/OS-keystore protection is queued), and the wallet's shielded side has a 64-spend one-time-key budget per master before you rotate it. In-circuit WOTS awaits its KAT campaign; agent-side proving on your own hardware assumes a GPU — the public prover does it for you on the testnet.
