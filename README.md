...
[**hashkinetics.org**](https://www.hashkinetics.org) · [**Live explorer**](https://www.hashkinetics.org/explorer) · [**Public RPC**](https://rpc.hashkinetics.org) · [**X @hashkinetics**](https://x.com/hashkinetics) · [**Discord**](https://discord.gg/RsSfb9gn3) · [**Telegram**](https://t.me/+tnRXX8KOCWA3YjE1) · validators@hashkinetics.org · [**Node Keys**](https://nodes.hashkinetics.org)

`testnet-1: LIVE since 2026-09-02` · `node v0.19.4 (client-only, security) · consensus v0.19.0 · wallet v0.15.0 (test USDC) · Android wallet v0.3.0 (test USDC)` · `11 seats, 7 external` · `seats change on the running chain (v0.14.0)` · `bootstrap governance: genesis seats weigh 4 from height 110,000 (v0.18.1)` · `issued assets with issuer controls (v0.15.0)` · `spend proof: 1.24 s (GPU)` · `274 tx/s storm-measured` · `testnet only`
...
An 11-seat public **testnet-1** runs since 2026-09-02 — real hash-signed BFT, pinned genesis, protocol fee bound into the genesis from block 1, shielded pool active (one pool per asset since height 190,000), ≈ 1,104,500 blocks at ~1.3 s/block by 2026-09-23 (1,104,514 at 14:55 UTC), the founding fleet on v0.19.4 (the latest release, 2026-09-22, security; rolled to v0.19.4 on 2026-09-23 (14:38–14:52 UTC, gateway first, every seat's leaf counter monotone across the restart)) (consensus rules v0.19.0; its predecessor staging-1 ran 107k blocks and is archived). Every release since launch has rolled onto it without a pause, one voter at a time — v0.13.2 (hardening), v0.14.0 (seat changes), v0.15.0 (issued assets), v0.17.0 (verify-only client), v0.18.1 (bootstrap governance), v0.19.0 (one pool per asset), through v0.19.3 (2026-09-13, client-only) — and seven external seats were admitted on the running chain between 2026-09-05 and 2026-09-12.
...
A frozen account's transfer comes back as `rejected: frozen by issuer`; a paused asset's as `rejected: asset paused` — receipts the explorer shows. What is *not* here yet: attested minting verified **in consensus** — since 2026-09-09 `USDC.sep` is minted by its issuer account against Sepolia USDC locked in `HKVault`, and a burn naming a Sepolia address is turned into an `unlock`, but the attestation is a 1-of-1 founder-run service off-chain (B1, `docs/BRIDGE-GUIDE.md`, labelled as such); the in-consensus attested-mint path with a bonded hash-signed relayer is designed, not built; every issued asset on testnet-1 is a test asset. Rule, gates and commitment layout: `docs/X1-ISSUED-ASSETS.md`. ⚠ The first asset transaction on testnet-1 makes v0.15.0 the minimum node version from that block.

## Run a full node on testnet-1 (now, no permission needed)

`networks/testnet-1/` holds the pinned genesis, the bootstrap peers and — since v0.15.1 — the three STARK verifying keys (`vks.json`, pinned by the genesis, so your node verifies every proof locally and never depends on our prover). Clone, build, point your node at them, and you're syncing and independently verifying the live chain, serving your own RPC and explorer — and, since v0.15.2, listed on the network's live roll call ([hashkinetics.org/network#live](https://www.hashkinetics.org/network#live), read from the gateway's own peer table through `hk_getPeers`) the moment you connect. One screen of commands, no GPU, no stake: `networks/testnet-1/README.md` (run the current release, v0.19.4 — client-only, security; v0.19.0 is the consensus minimum since height 190,000, and each consensus change raises the minimum at the block where it first applies). The genesis digest is the network's fingerprint; your node's `app_hash` at any height must equal what `rpc.hashkinetics.org` reports — same input, same hash, no trust required.
...
The testnet recruits external validators now — one Linux box, **no GPU, no stake, no cost** — and seven are seated: on 2026-09-05 an independent operator who had re-verified the chain from genesis on the public kit was admitted on the running chain by a certificate from 3 of the 4 founding seats' roots (block 72219, effective 72220), and on 2026-09-06 a second operator by 4 of 5 (block 96629, effective 96630 — every approval signed from a sealed key file); 6 seats since, no new genesis, chain never paused; on 2026-09-07 a third operator was seated in block 128613 by the four founding seats alone (the first admission under G1) — 11 seats, 7 external (the eighth seated 2026-09-09 in block 209,149; the ninth and tenth on 2026-09-11 in blocks 303,291 and 319,494, each after its stated app_hash matched the public chain; the tenth is the first professional operator, running PoS infrastructure since 2018; the eleventh on 2026-09-12 in block 379,143 — the last seat the founding roots can admit on their own under G1). **From height 110,000 (v0.18.1, G1 bootstrap governance) the four genesis seats weigh 4 each** — a published rule in the binary, not a certificate — so the founding fleet holds more than ⅔ on its own while the network is this young: an external seat can neither stall the chain by going down nor veto a set change, a new seat is admitted on founding approvals alone, and the weight comes back to external seats by `SetPower` certificate on a dated schedule with a receipt (`docs/V1-VALIDATOR-SET-CHANGES.md` §6). Your keys are generated on your machine and never leave it; your permanent identity is a stateless SLH-DSA root; exhaustion and downtime are liveness faults only, never fund-loss — proven the hard way: this network's val-0 ran its tree to zero, was revived by a root-signed cert carried through a peer (`issue-rotation` → `hk_submitRotation`), and rejoined with a fresh tree. The whole flow is in this repo.

At mainnet each node needs a Node Key — a software license, not a token; Node Keys are available at https://nodes.hashkinetics.org.

**v0.19.4 (2026-09-22, client-only, security — the consensus signer's state write is durable; every validator should restart on it once) is the release to run; the consensus rules are v0.19.0 (2026-09-09) — its activation height, 190,000, passed at ≈ 08:47 UTC on 2026-09-09 with all seven seats then aboard** (P6: one shielded pool per asset — a consensus change; a node still on v0.18.x islands at the first second-asset shield). After it, v0.19.2 (2026-09-13) lets the faucet drip issued assets for the wallets' test USDC (P6.2), and v0.19.3 (2026-09-13) makes a restarted validator always learn its peers' gossipsub subscriptions (N2 — a seat that re-dialed the gateway before its old socket died had proposed in round 1 for eight hours; operators: restart with a gap — stop, wait 45 s, start) — both client-only, no deadline. **The same morning the first bridge loop closed on the testnets: 20 Sepolia USDC locked → 20 `USDC.sep` minted on testnet-1 → 5 burned → 5 USDC released, every hop a public receipt (`docs/BRIDGE-GUIDE.md`; 1-of-1 attestor today, founder-run — said plainly there); that evening bridged USDC was shielded into its own pool, and an external operator crossed the bridge from the page unprompted. v0.19.1 (client-only, no deadline) carries the bridge subcommands and a CLI wallet that binds to a real account.** Before it, an external operator, Eddy, found and measured that every consensus signature cost ~0.45 s of CPU on v0.18.1 (a cache-validation bug in the vendored hash-signature crate plus a per-call key rebuild); v0.18.2 keeps the expanded key between signatures — 179 µs per signature on the release build, signatures byte-identical, no rule change, no deadline; the same release paces blocks to a 1 s floor (a proposer waits out `HK_MIN_BLOCK_INTERVAL_MS` since its last commit — unpaced, a devnet ran at 9 blocks/s), so the founding seats' own heights take 1.0 s (they run v0.19.4 since 2026-09-23); the chain's cadence is bounded by its slowest seat — ~1.3 s/block with 11 seats on 2026-09-14. Our first diagnosis (disk latency) was wrong and is corrected in the changelog; his name is on the release.

Read `docs/VALIDATOR-ONBOARDING.md`, then mail **validators@hashkinetics.org** with your `validator.json`. Honest framing: external operators start as **observers** (full verification, own RPC + explorer — same binary, minus the vote); since v0.14.0 a voting seat is **admitted on the running chain** by a certificate approved by more than ⅔ of the current seats' root keys — no new genesis (`docs/V1-VALIDATOR-SET-CHANGES.md`). An observer at the tip with our `app_hash`, running the current release, is the precondition; all seven external seats were admitted exactly that way — no ceremony, no restart. Since seat #11 (2026-09-12) every admission certificate carries an external co-signature by policy — the rule, checked against the current set, would still pass the twelfth on 16 of 23 and first forces it at the thirteenth — unless the `SetPower` handover comes first, as designed. How a genesis is run, with the record of this one: `docs/CEREMONY-TESTNET-1.md`.
...
| `chain/rehearsal.sh` · `chain/gate-*.sh` | The devnet gates every release passes: ceremony + restore shapes (`rehearsal.sh`), then one gate per feature — v1 25/25 · x1 40/40 · k6 16/16 · n1 20/20 · s 33/33 · h3 22/22 · r11 29/29 · g1 40/40 · wa1 19/19 · p6 48/48 · b1 50/50 · p6-2 31/31 · n2 18/18 |
...
- **Nothing is audited.** Scope is prepared (`docs/AUDIT-SCOPE.md`) and CertiK is engaged (2026-09-02); the audit campaign is a gate before any mainnet. The Node Key sale contract (`HKNodeKey`, Ethereum mainnet) has no independent audit yet either — self-reviewed and tested, verified source, an independent audit commissioned from the first proceeds, bug bounty up to 1 ETH.
- **No 30-day public soak yet** — testnet-1 launched 2026-09-02 (staging-1 before it ran six days and 107k blocks); seven external seats are aboard (the seventh since 2026-09-12 — 11 seats, total power 23, quorum 16), but the soak clock starts only after the **handover**: from height 110,000 the four genesis seats hold 16 of the 23 power by rule (v0.18.1), and the clock runs from the day that weight is lowered by certificate so that external seats hold at least ⅓ — a dated milestone, published in the changelog when it happens. Until then the founders can decide blocks and set changes alone, and we say so.
- **Issued assets are issuer-signed, not yet attested in consensus.** The registry, mint/burn/freeze/pause and conservation are live (v0.15.0); since 2026-09-09 `USDC.sep` is bridged from Sepolia (B1: USDC locked in `HKVault` → issuer mint on testnet-1; burn-with-destination → `unlock`; `docs/BRIDGE-GUIDE.md`) under a **1-of-1 founder-run attestor**, labelled as such on every surface — `USDC.sep` is never called "USDC"; the attested-mint path verified in consensus (an issuer's deposit attestation checked by every validator, paired with a bonded hash-signed relayer) is designed, not built; no stablecoin issuer has deployed — every issued asset on testnet-1 is a test asset.
- **Rotation hardening (R-series) is production-proven, not just built:** automatic threshold rotation has fired unattended across 45+ epochs; the peer-carried revival path has resurrected a fully-exhausted validator three times; certificates verify against the set as of their height, so syncing across rotation boundaries works (v0.10.7); catch-up verification is parallel (v0.10.8 — a joining machine measured 2→71 blocks/min, converging on a chain that adds ~27); since v0.10.9 syncing spends **zero** signing leaves — proven end-to-end when an independent observer re-verified the entire chain from genesis (~76,000 blocks, every vote and STARK) and reported the byte-identical state commitment with zero leaves spent; and since v0.10.10 a validator that falls behind **abstains instead of burning keys** (corroborated peer evidence gates all vote/proposal signing; gate receipt: a deliberately-wedged voter spent 2 leaves where the old behavior cost a fresh tree ~1,700, then rejoined byte-identical and voted at the first possible height) and the rotation threshold is checked on a **timer as well as on commits** — a parked node now rotates itself before exhaustion instead of after. Since v0.12.2 a rotation certificate that never landed is re-issued instead of wedging the validator (R9 — production-proven the day it shipped), and since v0.13.0 (R10 v2) a restart resumes at the chain height without rehydrating history — voting within seconds. testnet-1's own first rotation: all four seats crossed their thresholds and rotated unattended at heights 11,584–11,595 on 2026-09-03, zero blocks missed. And since v0.17.0 (R11) the node verifies STARKs with a verify-only client — the ~6.7 GB steady RSS and the 3–6-minute restart of earlier releases were the proof-system's *proving* engine built just to verify; every node now reports its own resident set and start-up cost in `hk_chainInfo.process`; measured on the four founding seats after the 2026-09-06 roll: 54–59 MiB resident, 6–16 s from start to an answering RPC (the v0.17.0 changelog entry has the per-seat receipt), so the operator requirement is now 4 GB RAM minimum, 8 GB comfortable — the line followed the measurement, not the other way round.
- **R16 (external report 2026-09-22, fixed in v0.19.4):** the signer's state write now fsyncs its directory and restart resumes upwards — no reuse under any failure that preserves the last durable write (the directory entry is fsynced too since v0.19.4 — R16); a floor from the node's own evidence and a reservation window are the follow-ups. A reused LM-OTS leaf leaks one-time key material; one reuse is not enough to forge, but repeated reuse of the same leaf (a crash-looping node) makes forgery cheap.
...
- The faucet drips test units with no monetary value; the treasury was allocated in the testnet-1 genesis. Every transaction pays a flat 100-micro protocol fee (burned) — an anti-spam floor and a mechanism rehearsal, not a fee market. No token exists and none is sold. What is sold is Node Keys — software licenses to run one node on the HashKinetics network at mainnet — at https://nodes.hashkinetics.org (ERC-721 `HKNodeKey`, Ethereum mainnet `0x36edb45c4610AE2A7FE6334343D9D958Fb81EC84`, source verified on Etherscan and Sourcify; not independently audited yet — self-reviewed and tested, an independent audit is commissioned from the first proceeds, bug bounty up to 1 ETH).
...
Found something? **security@hashkinetics.org.** The Node Key sale contract carries a bug bounty of up to 1 ETH; a funded program for the chain comes with the audit campaign. We accept reports, not code: security findings and reproductions are credited; every fix and its tests are written in-house. Reports are credited, taken seriously, and answered by a human who reads code.
